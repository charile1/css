...
'1, 2, 3, 4, 8, 1, 2, 3, 4, 8, 1, 2, 3, 4, 8, 1, 2'.split('8');

'1, 2, 3, 4, 8, 1, 2, 3, 4, 8, 1, 2, 3, 4, 8, 1, 2'.split('8').length - 1;

// length - 1 하는 이유
'1, 2, 3, 4, 8, 1, 2, 3, 4'.split('8')
//  ['1, 2, 3, 4, ', ', 1, 2, 3, 4']
// 원래 8의 갯수는 1개
// 8 기준으로 쪼개지면 length는 2개 
// 2 - 1 = 1 
// 가래떡을 4번 썰면 5덩이가 되니까 1을 빼는 거군요
...
// constructor 자료구조의 근간이 되는 데이터들을 넣는 곳 ,데이터를 셋팅하는애 
// push로  어레이에 데이터를 푸쉬해줌 
// this는 인스턴스를 향하게 하려고
// pop도 역시 꺼내는 건데 파이썬처럼 pop에 대한 값만 빼게 한 대표님의 욕심! 
// pop(index=this.arr.length-1) 와 아무것도 없는 경우 2가지 경우
// 1. s.pop() , 2.s.pop(마지막값) 
// index에 해당하는 위치의 아이를 1개 delete하겠다.
// pop은 꺼낸 값을 리턴해주므로 
...
// 5.1 트리 
// 트리의 기본형태
// 각 노드에겐 value, child-left, child-right 가 있어야 한다. 
// 오브젝트 먼저 이해하고 갑시다.
const tree = {
    root: {
        value: 5,
...
// object로 linked list와 tree를 만들수 있는데 굳이 class로 만드는 이유는
// 1. 확장성(예제를 보여드리겠습니다.)
...
// || - or (논리합, 더했을때 0 외에 다른 숫자들은 모두 1)
// && - and (논리곱, 곱했을 때 0 외에 다른 숫자들은 모두 1)
// | - not
