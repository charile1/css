...
export default function example() {
	...
	const sampleModels = [];
	sampleModels.push(new SampleModel({ modelSrc: '/models/roncho.glb', scene, gltfLoader, x: -5, z: 20 }));
	sampleModels.push(new SampleModel({ modelSrc: '/models/iphone.glb', scene, gltfLoader, x: 7, z: 10 }));
	sampleModels.push(new SampleModel({ modelSrc: '/models/roncho.glb', scene, gltfLoader, x: -10, z: 0 }));
	...
	function draw() {
		...
		sampleModels.forEach(item => {
			if (item.mesh) {
				item.mesh.position.y = Math.sin(time * 100) * 0.02;
			}
		});
